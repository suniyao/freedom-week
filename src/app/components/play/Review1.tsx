...
export default function Review() {
  ...
  const milliseconds_spent = (playSession.endTime as number) - (playSession.startTime as number);
  ...
  return (
    <div className="min-h-screen w-full bg-amber-50 text-stone-900 py-16 px-4 sm:px-8">
      <div className="mx-auto space-y-6 flex flex-col items-center">
        ...
        <div className="flex flex-col gap-6 p-3 bg-amber-300 rounded-xl">
          <div className="m-1 bg-amber-50 p-3 rounded-xl text-center items-center">
            <span className="text-xl">Score</span>
            <span className="text-xl font-bold">{playSession.calculateScore()}</span>
          </div>
          <div className="m-2 bg-amber-50 p-3 rounded-xl">Percentage correct: 
            <span className="text-xl text-red-500">{correct_percentage}%</span>
          </div>
          <div className="m-2 bg-amber-50 p-3 rounded-xl">Time spent: 
            <span className="text-xl">{minutesSpent}:{secondsSpent < 10 ? "0" + secondsSpent : secondsSpent}</span>
          </div>
